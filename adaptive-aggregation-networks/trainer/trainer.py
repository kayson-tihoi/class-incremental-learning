...
class Trainer(BaseTrainer):
    def train(self):
        ...
        # set logger
        logger = create_logger(self.save_path)
        ...
        for iteration in range(start_iter, int(self.args.num_classes/self.args.nb_cl)):
            ...
            logger.info('Check point name: ' + ckp_name)
                ...
                if iteration > start_iter:
                    ...
                    if self.args.baseline == 'lucir':
                        b1_model, b2_model = incremental_train_and_eval_lucir(self.args, self.args.epochs, self.fusion_vars, \
                            ...
                            X_protoset_cumuls, Y_protoset_cumuls, order_list, cur_lambda, self.args.dist, self.args.K, self.args.lw_mr, balancedloader, logger=logger)    
                        ...
                        b1_model, b2_model = incremental_train_and_eval_icarl(self.args, self.args.epochs, self.fusion_vars, \
                            ...
                            self.args.icarl_T, self.args.icarl_beta, logger=logger)
                    else:
                    ...
                    b1_model = incremental_train_and_eval_zeroth_phase(self.args, self.args.epochs, b1_model, \
                        ...
                        cur_lambda, self.args.dist, self.args.K, self.args.lw_mr, logger=logger) 
            ...
            logger.info('Computing average accuracy...')
            logger.info("  Average accuracy (FC)         :\t\t{:.2f} %".format(avg_cumul_acc_fc))
            logger.info("  Average accuracy (Proto)      :\t\t{:.2f} %".format(avg_cumul_acc_icarl))
