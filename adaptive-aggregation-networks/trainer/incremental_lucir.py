...
def incremental_train_and_eval(the_args, epochs, fusion_vars, ref_fusion_vars, b1_model, ref_model, b2_model, ref_b2_model, \
    ...
    K, lw_mr, balancedloader, fix_bn=False, weight_per_class=None, device=None, logger=None):

    logger_fn = logger.info if logger is not None else print
    ...
    for epoch in range(epochs):
        ...
        logger_fn(f'Epoch: {epoch}, learning rate: {tg_lr_scheduler.get_lr()[0]}')
        ...
        logger_fn('Train set: {}, train loss1: {:.4f}, train loss2: {:.4f}, train loss3: {:.4f}, train loss: {:.4f} accuracy: {:.4f}'.format(len(trainloader), train_loss1/(batch_idx+1), train_loss2/(batch_idx+1), train_loss3/(batch_idx+1), train_loss/(batch_idx+1), 100.*correct/total))
        ...
        logger_fn('Test set: {} test loss: {:.4f} accuracy: {:.4f}'.format(len(testloader), test_loss/(batch_idx+1), 100.*correct/total))

    logger_fn("Removing register forward hook")
